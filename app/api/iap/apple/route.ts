import { NextResponse } from "next/server";
import { requireBearerUid } from "@/lib/auth-api";
import {
  saveAppleSubscription,
  verifyAppleReceipt,
} from "@/lib/iapAppleService";

export const runtime = "nodejs";

export async function POST(request: Request) {
  const auth = await requireBearerUid(request);
  if (auth instanceof NextResponse) return auth;
  const { uid } = auth;

  let body: { purchaseToken?: string };
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ error: "JSON invalide." }, { status: 400 });
  }

  if (!body.purchaseToken || typeof body.purchaseToken !== "string") {
    return NextResponse.json(
      { error: "purchaseToken est requis." },
      { status: 400 }
    );
  }

  try {
    const verification = await verifyAppleReceipt(body.purchaseToken, uid);
    if (!verification.isValid || !verification.expiresDate) {
      return NextResponse.json(
        { error: "Vérification Apple impossible ou rejetée." },
        { status: 400 }
      );
    }

    await saveAppleSubscription(uid, verification, body.purchaseToken);

    const isPremium = verification.expiresDate.getTime() > Date.now();

    return NextResponse.json({
      success: true,
      isPremium,
      expiryDate: verification.expiresDate.toISOString(),
    });
  } catch (e: unknown) {
    const message =
      e instanceof Error ? e.message : "Erreur lors de la validation IAP.";
    console.error("[iap/apple]", e);
    return NextResponse.json({ error: message }, { status: 400 });
  }
}
